...
func FetchLatestKomik(c *fiber.Ctx) error {
	page := c.Query("page")
	if page == "" {
	...
	err := collector.Visit(util.BASE_URL + "/project-list/page/" + page)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).SendString("Failed to fetch data")
	}

	currentPageFloat, _ := strconv.ParseFloat(currentPage, 64)
	lengthPageFloat, _ := strconv.ParseFloat(lengthPage, 64)

	return c.JSON(fiber.Map{
		"status":       "success",
		"current_page": currentPageFloat,
